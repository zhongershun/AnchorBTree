...
void CopyString(char *dest, const std::string &src) {
  snprintf(dest, DASET_PAGE_SIZE, "%s", src.c_str());
}
...
TEST(BufferPoolManagerTest, PagePinEasyTest) {
  ...
  {
    auto page0_write = bpm->WritePage(pageid0);
    ...
    auto page1_write = bpm->WritePage(pageid1);
    ...
    // const auto temp_page1_opt = bpm->ReadPage(temp_page_id1);
    // ASSERT_FALSE(temp_page1_opt.has_value());

    // const auto temp_page_id2 = bpm->NewPage();
    // const auto temp_page2_opt = bpm->WritePage(temp_page_id2);
...

  {
    ...
    ASSERT_EQ(bpm->GetPinCount(pageid0),0);
    ASSERT_EQ(bpm->GetPinCount(pageid1),0);
  }
  ...
  {
    auto page0_read = bpm->ReadPage(pageid0);
    ...
    auto page1_read = bpm->ReadPage(pageid1);
...
TEST(BufferPoolManagerTest, PageAccessTest) {
  ...
  auto disk_manager = std::make_shared<DiskManager>(db_fname);
  auto bpm = std::make_shared<BufferPoolInstance>(0, 1, disk_manager.get());

  const auto pid = bpm->NewPage();
  char buf[DASET_PAGE_SIZE];
  ...
  for (size_t i = 0; i < rounds; i++) {
    ...
    memcpy(buf, guard.GetData(), DASET_PAGE_SIZE);
